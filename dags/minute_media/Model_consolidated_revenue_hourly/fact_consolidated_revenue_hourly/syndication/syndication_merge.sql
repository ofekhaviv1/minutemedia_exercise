...
using `minute-media-490214`.`minute_media_STG`.`syndication_explode_data` as source
on
  ...
  -- Strict grain match (COALESCE on nullable dims)
...
when matched then
  update set
    ...
    target.max_syndication_date = source.max_syndication_date,
