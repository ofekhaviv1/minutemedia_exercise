-- DQ check: every syndication STG row should have a matching SSOT row with the same actual_revenue.
-- Uses a grain-level JOIN (not date-level) to avoid counting old syndication rows from prior runs.
with stg_total as (
  select round(sum(actual_revenue), 4) as total
  from `minute-media-490214.minute_media_STG.syndication_explode_data`
),

matched_total as (
  select round(sum(t.actual_revenue), 4) as total
  from `minute-media-490214.minute_media_STG.syndication_explode_data` s
